...
int nextUtf8Index(const String &text, int index) {
...
bool initFontSystem(TFT_eSPI &tft, U8g2_for_TFT_eSPI &u8g2, const uint8_t *fallbackFont) {
  ...
  if (!SD.exists(SD_FONT_FILE)) {
    Serial.printf("[Font] Missing %s, use fallback font\n", SD_FONT_FILE);
    u8g2.begin(tft);
    return false;
  }

  File fontFile = SD.open(SD_FONT_FILE);
  if (fontFile) {
    const uint32_t fontBytes = fontFile.size();
    Serial.printf("[Font] Found %s (%u bytes)\n", SD_FONT_FILE, fontBytes);
    fontFile.close();

    if (fontBytes > SD_FONT_MAX_BYTES) {
      Serial.printf("[Font] File too large for ESP32 RAM (max %u bytes), use fallback font\n",
                    SD_FONT_MAX_BYTES);
      u8g2.begin(tft);
      return false;
    }
  ...
  tft.loadFont(SD_FONT_NAME, SD);
  sdFontReady = true;
  Serial.printf("[Font] Loaded SD font %s\n", SD_FONT_FILE);
  return true;
}
...
int fontUtf8WrapIndex(const String &text, int maxWidth) {
  ...
  while (pos < (int)text.length()) {
    ...
    const String trial = text.substring(0, next);
    if (fontTextWidth(trial.c_str()) > maxWidth) {
      break;
    }
    cut = next;
    pos = next;
